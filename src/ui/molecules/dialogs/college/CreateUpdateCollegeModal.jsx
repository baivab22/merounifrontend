...
const CreateUpdateCollegeModal = ({
      ...
      const fetchCollegeData = async () => {
        try {
          ...
          if (collegeData.degrees && Array.isArray(collegeData.degrees)) {
            const degreeIds = collegeData.degrees.map((degree) =>
              String(degree.id)
            )
          ...
          if (collegeData.collegeAddress) {
            ...
            setValue(
              'address.postal_code',
              collegeData.collegeAddress.postal_code
            )
          }
  ...
  // Effect to fetch programs when selected degrees change (programs_degrees only; optionally filtered by university)
  useEffect(() => {
  ...
  const handleSave = async (data, status = 'published') => {
    try {
      ...
      const uniIds = (data.university_id || [])
        .map((id) => parseInt(id))
        .filter((id) => !isNaN(id))
                    ...
                    <div>
                      ...
                      <input
                        ...
                        {...register('university_id', {
                          required: 'Please select at least one university'
                        })}
                      />
                    </div>
                    ...
                    <div className='grid grid-cols-1 sm:grid-cols-2 gap-8'>
                      ...
                      <div>
                        ...
                        {selectedUniIds.length === 0 ? (
                          ...
                          selectedDegreeIds.length === 0 && (
                            <p className='text-[10px] text-gray-400 mt-2 font-medium bg-gray-50 p-2 rounded-md border border-dashed border-gray-200'>
                              Please select at least one degree to view
                              available programs.
                            </p>
                          )
                        )}
