      ...
      <main className='bg-white min-h-screen pb-20'>
          ...
          <div className='container mx-auto px-4 py-16 md:py-24 relative z-10'>
            <div className='max-w-4xl'>
              ...
              >
                <Link
                  href='/programs'
                  className='group flex items-center gap-2 text-sm font-bold text-[#0A6FA7] hover:text-[#085e8a] transition-all bg-white px-4 py-2 rounded-full shadow-sm border border-[#0A6FA7]/10'
                >
              ...
              >
                <div className='flex flex-wrap items-center gap-3 mb-6'>
                  {program.code && (
                    <span className='px-3 py-1 bg-[#0A6FA7] text-white text-[10px] font-black uppercase tracking-widest rounded shadow-lg shadow-[#0A6FA7]/20'>
                      {program.code}
                    </span>
                  )}
                  {program.programdegree?.title && (
                    <span className='bg-white px-3 py-1 rounded-md text-[#0A6FA7] text-xs font-bold border border-[#0A6FA7]/10 shadow-sm'>
                      {program.programdegree.short_name || program.programdegree.title}
                    </span>
                  )}
                  {program.programlevel?.title && (
                    <span className='text-gray-400 text-xs font-medium bg-gray-100/50 px-2 py-1 rounded'>
                      {program.programlevel.title}
                    </span>
                  )}
                </div>
        ...
        <div className='container mx-auto px-4 py-16'>
          <div className='grid grid-cols-1 lg:grid-cols-3 gap-16'>
            {/* Left Sidebar or Main Content depending on width */}
                    ...
                    {Object.entries(groupedSyllabus)
                      ...
                      .map(([year, semesters]) => (
                        <div key={year} className='relative pl-8 border-l-2 border-[#0A6FA7]/10 pb-4 last:pb-0'>
                          ...
                          <h3 className='text-xl font-black text-[#0A6FA7] mb-8'>
                            Year {year}
                          </h3>
                          <div className='grid grid-cols-1 md:grid-cols-2 gap-8'>
                            {/* Yearly courses if any (semester 0) */}
                            {semesters.yearly && (
                            ...
                            {Object.entries(semesters)
                              ...
                              .map(([semester, items]) => (
                                <div key={semester} className='space-y-4'>
                                  <h4 className='text-xs font-black text-gray-400 uppercase tracking-[0.2em] px-1'>
                                    Semester {semester}
                                  </h4>
                                  <div className='space-y-2'>
                                    {items.map((item) => (
                                      <div
                                        key={item.id}
                                        className='flex justify-between items-center p-4 bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-md hover:border-[#0A6FA7]/20 transition-all group'
                                      >
                                        <span className='font-bold text-gray-700 group-hover:text-gray-900 transition-colors'>
                                          {item.programCourse?.title ?? '—'}
                                        </span>
                                        {item.is_elective && (
                                          <span className='text-[10px] text-amber-600 bg-amber-50 px-2 py-0.5 rounded font-black uppercase'>
                                            Elective
                                          </span>
                                        )}
                                      </div>
                        ...
                        <div key={college.id} className='group'>
                          {college.slugs ? (
                            <Link
                              ...
                              className='flex items-center justify-between py-3 px-4 rounded-xl border border-gray-50 bg-gray-50/50 hover:bg-white hover:border-[#0A6FA7] hover:shadow-xl hover:shadow-[#0A6FA7]/5 transition-all text-gray-800'
                            >
                            ...
                            <div className='py-3 px-4 rounded-xl border border-gray-50 bg-gray-50/50 text-gray-800 font-bold text-sm'>
                              {college.name}
                            </div>
                          )}
                        </div>
