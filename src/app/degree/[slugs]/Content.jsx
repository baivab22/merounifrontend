...
const DegreeContent = ({ degree, error, slugs }) => {
        ...
        <div className='bg-white min-h-screen'>
          <div className='w-full bg-gray-50 border-b border-gray-100 py-16 md:py-20 relative'>
            <div className='absolute top-6 left-6 md:left-24 z-10'>
              <Link href='/degree' className='inline-flex items-center gap-2 px-4 py-2 bg-white/80 hover:bg-white backdrop-blur-md text-gray-700 rounded-full text-sm font-medium transition-all shadow-sm border border-gray-100' >
            ...
            <div className='container mx-auto px-4'>
              <div className='max-w-4xl mx-auto'>
                {degree.short_name && <p className='text-xs font-bold text-[#30AD8F] uppercase tracking-[0.2em] mb-3'> {degree.short_name} </p>}
                <h1 className='text-3xl md:text-5xl font-bold text-gray-900 mb-6'> {degree.title} </h1>
                {degree.description && <p className='text-gray-600 text-lg max-w-2xl leading-relaxed italic'> {degree.description} </p>}
              </div>
            </div>
          </div>
          ...
          <div className='container mx-auto px-4 pt-16'>
            <div className='max-w-4xl mx-auto'>
              <div className='w-full aspect-video rounded-2xl overflow-hidden shadow-lg border border-gray-100 bg-gray-50 flex items-center justify-center mb-16'>
                <img src={degree.cover_image || degree.featured_image || '/images/logo.png'} alt={degree.title} className={degree.cover_image || degree.featured_image ? 'w-full h-full object-cover' : 'w-1/3 h-auto object-contain opacity-50'} />
              </div>
              {degree.content && ( <div className='mb-20'> <HTMLRenderer html={degree.content} /> </div> )}
              {degree.programs && degree.programs.length > 0 && (
                <div className='mb-20'>
                  <h2 className='text-2xl font-bold text-gray-900 mb-8 border-l-4 border-[#30AD8F] pl-4'> Available Programs </h2>
                  <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
                    {degree.programs.map((program) => ( <ProgramCard key={program.id} program={program} /> ))}
                  </div>
                </div>
              )}
            </div>
